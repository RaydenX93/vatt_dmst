...
for i=1:expected_sims
    ...
    if exist([dirname '\' output_name '.mat'],'file') ~= 0
        ...
        [data_post, ~, ~, ~, ~, ~, ~, ~] = vatt_dmst([ave_data{lon,lat}(:,1), ave_data{lon,lat}(:,2)], [dirname '\' output_name]);
    end
...
%% Save output %%
save([dirname '\' output_name '_sub_finished.mat'],'long_coord','lat_coord','cp','p','dmst_time','run_number','good_cells','turb_mask','run_mask');
